...
# ── Legacy aliases (current FE: /login + /callback → Keycloak) ──────────────


@auth_router.get("/login")
async def login_legacy() -> RedirectResponse:
    idp = get_provider("keycloak")
    state = secrets.token_urlsafe(32)
    redirect_uri = config.AUTH_CALLBACK_URL  # old URI
    ...
    response.set_cookie(
        config.OAUTH_PROVIDER_COOKIE, "keycloak", max_age=600, **_cookie_kwargs()
    )
    return response
