# auth_routes.py - for the authentication routes for the backend  fastapi-keycloak for redirecting the login to keycloak and getting the tokens/cookies for the frontend.
...
async def callback(
    ...
    # fastapi-keycloak recieves token code from keyclock not actual token , so we need to exchange the code for the token.
